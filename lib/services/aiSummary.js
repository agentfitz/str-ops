...
export async function generateSummary(reportData, hostContext = '') {
  ...
  const payoutLine = account_balance.has_balance
    ? `Owner payout this month: ${fmt(account_balance.owner_payout)} (combined balance ${fmt(account_balance.combined_balance)} minus management fee and ${fmt(account_balance.operating_minimum)} operating reserve)`
    : 'Account balance not yet entered — payout not yet calculated'

  const nicknamePool = (owner.nickname || owner.name.split(' ')[0])
    .split(',').map(n => n.trim()).filter(Boolean)
  const ownerHandle = nicknamePool[Math.floor(Math.random() * nicknamePool.length)]
...
Write 2–3 sentences. Max 400 characters total. Address ${ownerHandle} by name once, naturally.
